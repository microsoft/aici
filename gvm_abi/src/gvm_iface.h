...
// Called once per module, to get a AICI for a specific query
Aici *aici_create(void);

// If a query is split into several (eg., during beam-search, or when returning several results)
// this is called to get AICI for the sub-query.
Aici *aici_clone(Aici *parent);

// These two are called after aici_create() and aici_clone() on the fresh AICI.
...
// This is called for AICIs that no longer needed (eg. because generation completed,
// or beam-search branch was cut).
void aici_free(Aici *aici);
