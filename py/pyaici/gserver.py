...
from pyaici.gserver_native import (
    TokenSet,
    tokenize,
    detokenize,
    RegexConstraint,
...
import pyaici.gserver_native as _aici
...
async def gen_tokens(
    ...
    constraint: Optional[Callable[[], Constraint]] = None,
    max_tokens=20,
) -> List[Token]:
    """
    ...
    """
    res: List[Token] = []
    assert len([x for x in [regex, options, yacc, substring, constraint] if x is not None]) <= 1
    if regex is not None:
        ...
        next_token = ConstrainedToken(constraint)
    else:
