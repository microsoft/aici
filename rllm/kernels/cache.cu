...
extern "C"
void gather_scatter_inner_bf16(
  __nv_bfloat16* __restrict__ key,     // [num_tokens, num_heads, head_size]
  __nv_bfloat16* __restrict__ value,   // [num_tokens, num_heads, head_size]
  __nv_bfloat16* __restrict__ key_cache,     // [num_blocks, num_heads, head_size/x, block_size, x]
  __nv_bfloat16* __restrict__ value_cache,   // [num_blocks, num_heads, head_size, block_size]
  const int* __restrict__ slot_mapping, // [num_tokens]
  const int key_stride,
  const int value_stride,
  const int num_heads,
  ...
  const int op)
{
  ...
  const cudaStream_t stream = 0;
  if (op == 0)
  reshape_and_cache_kernel<__nv_bfloat16><<<grid, block, 0, stream>>>(
        key,
        value,
        key_cache,
        value_cache,
     ...
     gather_cached_kv_kernel_optimized<__nv_bfloat16><<<grid, block, 0, stream>>>(
        key,
        value,
        key_cache,
        value_cache,
